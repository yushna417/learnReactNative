        ...
        <ScrollView contentContainerStyle={{ flexGrow: 1 }}>
          <Box className="flex-1 px-4 py-8">
            <Center className="mb-8 mt-10">
              ...
              <Text className="text-gray-500 text-base">Please login to continue</Text>
            </Center>
        ...
        <Box className="flex-1 px-5 pt-6 pb-10">
          ...
          <Card className="bg-white rounded-3xl p-6 mb-6 shadow-md border border-yellow-100">
            <Center>
              ...
              <Box className="relative mb-4">
                ...
                <Avatar size="2xl" className="bg-yellow-500 border-4 border-white shadow-lg">
              ...
              {/* Status Badge */}
              <Badge
                className={`mt-2 px-3 py-1 rounded-full ${user.is_active ? 'bg-emerald-100' : 'bg-red-100'
                  }`}
              >
                <HStack className="items-center">
                  <Box
                    className={`w-2 h-2 rounded-full mr-2 ${user.is_active ? 'bg-emerald-500' : 'bg-red-500'
                      }`}
                  />
                  <BadgeText
                    className={`text-xs font-semibold ${user.is_active ? 'text-emerald-700' : 'text-red-700'
                      }`}
                  >
          ...
          <Card className="bg-white rounded-3xl p-5 mb-6 shadow-md border border-yellow-100">
            <VStack space="lg">
              ...
              {[
                ...
                {
                  icon: <FontAwesome6 name="calendar-day" size={20} color="#ca8a04" />,
                  label: "Member Since",
          ...
          <Card className="bg-white rounded-3xl p-5 mb-8 shadow-md border border-yellow-100">
            <HStack className="justify-between items-center">
              ...
              <HStack className="items-center">
                <MaterialCommunityIcons name="account-tag" size={20} color="#eab308" />
              ...
              <Badge
                className={`px-3 py-1 rounded-full ${user.is_staff ? 'bg-yellow-200' : 'bg-gray-100'
                  }`}
              >
                <BadgeText
                  className={`text-xs font-semibold ${user.is_staff ? 'text-yellow-800' : 'text-gray-600'
                    }`}
                >
