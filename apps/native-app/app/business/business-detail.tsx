		...
		updateListing(data, {
			...
			onError: (error: any) => {
				...
				if (d?.title) setError("title", { message: d.title[0] });
					...
					setError("root", {
						message: d?.error || d?.detail || "Update failed.",
					});
			},
		});
	};
					...
					<VStack space="lg">
						...
						<FormControl isRequired isInvalid={!!errors.service_detail}>
							...
							<Controller
								...
								render={({ field: { onChange, value } }) => (
									<View
										className={`border rounded-xl bg-gray-50 ${errors.service_detail ? "border-red-300" : "border-gray-200"}`}
									>
										<RNTextInput
...
export default function BusinessDetail() {
	...
	const {
		...
		isError,
		refetch,
	} = useListingDetail(Number(businessId));
		...
		Alert.alert(
			...
			[
				{ text: "Cancel", style: "cancel" },
				{
					...
					style: "destructive",
					onPress: () => {
						deleteListing(Number(businessId), {
