...
const RichTextEditor = ({
	...
	return (
		<View
			className={`border rounded-xl bg-gray-50 ${error ? "border-red-300" : isFocused ? "border-yellow-400" : "border-gray-200"}`}
		>
			<RNTextInput
					...
					<View className="absolute bottom-0 left-0 right-0 bg-white rounded-t-3xl pb-8">
						...
						<ScrollView className="max-h-80">
							{CATEGORIES.map((cat) => (
								...
								>
									<Text
										className={`text-base ${value === cat ? "text-yellow-600 font-semibold" : "text-gray-700"}`}
									>
										{cat}
									</Text>
								</TouchableOpacity>
	...
	const onSubmit = (data: BusinessFormData) => {
		createListing(data, {
			...
			onError: (error: any) => {
				...
				else if (d?.business_category)
					setError("business_category", { message: d.business_category[0] });
				...
				>
					<View className="px-6 pt-4 pb-8">
						...
						<VStack space="lg">
							...
							<FormControl
								isRequired
								isInvalid={!!errors.latitude || !!errors.longitude}
							>
								<FormControlLabel className="mb-2">
									<FormControlLabelText className="text-gray-700 font-semibold text-sm">
										Business Location
