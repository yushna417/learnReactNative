					...
					{PER_PAGE_OPTIONS.map((n) => (
						<TouchableOpacity
							...
							className={`px-3 py-1.5 rounded-full ${
								perPage === n ? "bg-yellow-500" : "bg-gray-100"
							}`}
						>
							<Text
								className={`text-xs font-semibold ${
									perPage === n ? "text-white" : "text-gray-600"
								}`}
							>
								{n}
							</Text>
			...
			<View className="flex-row items-center justify-center gap-3 mb-5">
				<TouchableOpacity
					...
					className={`px-4 py-2 rounded-xl ${
						pagination.has_previous && !isFetching
							? "bg-yellow-500"
							: "bg-gray-100"
					}`}
				>
						...
						{Array.from({ length: total_pages }, (_, i) => i + 1).map((p) => (
							<TouchableOpacity
								...
								className={`w-7 h-7 rounded-full items-center justify-center ${
									p === page ? "bg-yellow-500" : "bg-gray-100"
								}`}
							>
								<Text
									className={`text-sm font-semibold ${
										p === page ? "text-white" : "text-gray-600"
									}`}
								>
									{p}
								</Text>
				...
				<TouchableOpacity
					...
					className={`px-4 py-2 rounded-xl ${
						pagination.has_next && !isFetching ? "bg-yellow-500" : "bg-gray-100"
					}`}
				>
					<Text
						className={`text-xs font-semibold ${
							pagination.has_next && !isFetching
								? "text-white"
								: "text-gray-300"
						}`}
					>
